...
for layer in game_data.get_layers(layer_type="tile"):
    print(layer.name, layer.z_index)`}
      />
      <p>
        The raw parsed structure lives on <code>game_data.parsed</code> (see
        the API reference's parsed data classes). For collision you usually
        want the layer as a flat grid:{" "}
        <code>game_data.build_tile_map()</code> collapses the tile layers into{" "}
        <code>{"{"}(col, row): tile_id{"}"}</code>, which is what{" "}
        <code>PhysicsWorld</code> owns.
      </p>
...
# stats: LayerRenderStats: drawn_tiles, skipped_tiles, visible_layers`}
      ...
      <ul>
        <li>
          <code>render(target, camera_xy, viewport_size=None, *, extra_objects=None, current_time_ms=None)</code>:{" "}
          <code>current_time_ms</code> drives tileset animations.
        </li>
        <li>
          <code>extra_objects</code> lets you blit sprites in the same pass
          (any objects with <code>surface</code>, <code>x</code>,{" "}
          <code>y</code>), after the tile layers.
        </li>
        <li>
          <code>warm_cache()</code> pre-bakes every tile variant up front,
          then frees the source data.
        </li>
      </ul>
      <Callout kind="tip" title="TILE CACHING">
        The renderer caches baked tile variants internally. If you swap
        tileset textures at runtime, call{" "}
        <code>clear_texture_caches()</code> to drop the cached surfaces, then
        let the renderer re-bake on demand.
      </Callout>
      ...
      <p>
        ...
        or air. Collision facts live in the world —{" "}
        <code>world.tile_map</code> plus the collision tileset. The split is
        deliberate: render everything, collide with a subset (
        <code>exclude_layers</code> only affects the world).
      </p>
      <p>
...
if any(s.one_way for s in tile_data.shapes):
    # dashed top edge at (tx * world.tile_size[0], ty * world.tile_size[1])`}
      ...
      <p>
        ...
        <code>ObjectCollisionManager</code>. <code>load_map_objects</code>{" "}
        takes the map <em>and a directory</em> containing the matching{" "}
...
if player_start is not None:
    player.x, player.y = player_start.x, player_start.y`}
      ...
      <ul>
        ...
        <li>
          <code>require_collision=True</code> (the default) returns only
          objects that have matching collision regions; pass{" "}
          <code>False</code> to also get visual-only objects (with empty
          shapes).
        </li>
        <li>
