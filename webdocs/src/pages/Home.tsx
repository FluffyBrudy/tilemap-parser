    ...
    >
      <section className="grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
        <div>
          ...
          <h1 className="max-w-3xl text-4xl font-semibold text-zinc-50 md:text-5xl">
            Load maps, render tiles, animate sprites, and resolve collision from one package.
          </h1>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-zinc-400">
            tilemap-parser is a practical bridge between tilemap-editor JSON and
            pygame projects. The docs focus on the path from installed package
            to a working top-down demo, with enough reference material to keep a
            real project moving.
          </p>
        ...
        <div className="overflow-hidden rounded-lg border border-zinc-800 bg-zinc-950">
          <div className="border-b border-zinc-800 px-4 py-3 text-sm font-medium text-zinc-300">
            Map, animation, collision
          </div>
          <div className="grid gap-0 sm:grid-cols-[0.85fr_1.15fr]">
            <div className="flex items-center justify-center border-b border-zinc-800 bg-zinc-900/60 p-6 sm:border-b-0 sm:border-r">
              <img
                src={heroImage}
                alt="Tilemap parser demo artwork"
                className="h-auto max-h-72 w-full max-w-64 object-contain"
              />
            </div>
            <div className="p-5">
              <CodeBlock
                title="game.py"
                code={`from tilemap_parser import (
    load_map, TileLayerRenderer,
    CollisionRunner, CollisionCache,
    SpriteAnimationSet, AnimationPlayer
)

data = load_map("data/map.json")
renderer = TileLayerRenderer(data)
animations = SpriteAnimationSet.load("data/player.anim.json")
runner = CollisionRunner.from_game_type(
    "topdown", CollisionCache(), data.tile_size
)`}
              />
            </div>
          </div>
